...
def update_operation_file(operation_file, yaml_content, schemas, operation_id):
    ...
    if desc_start and desc_end:
        ...
        desc_reference = f'```yaml\n--8<-- "src/spec/rest.yaml:{desc_start}:{desc_end}"\n```'
    ...
    if 'additional_schemas' in schemas:
        ...
        for schema_name, description in schemas['additional_schemas'].items():
            ...
            if start and end:
                section = f"\n### {description}\n\n```yaml\n--8<-- \"src/spec/rest.yaml:{start}:{end}\"\n```"
...
def update_rest_operation_file(rest_file, yaml_content, operation_id):
    ...
    if start and end:
        ...
        pattern = r'(--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'
        new_ref = f'\\g<1>{start}:{end}\\g<2>'
...
def update_operations_index_errors(index_file, yaml_content):
    ...
    error_mappings = [
        ('ErrorResponse', r'(## Error Response Model.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'),
        ('BadRequestErrorResponse', r'(### 400 - Bad Request Error Response.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'),
        ('UnauthorizedErrorResponse', r'(### 401 - Unauthorized Error Response.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'),
        ('ForbiddenErrorResponse', r'(### 403 - Forbidden Error Response.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'),
        ('NotFoundErrorResponse', r'(### 404 - Not Found Error Response.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'),
        ('UnsupportedOperationErrorResponse', r'(### 406 - Unsupported Operation Error Response.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'),
        ('ConflictErrorResponse', r'(### 409 - Conflict Error Response.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'),
        ('ServiceUnavailableErrorResponse', r'(### 503 - Service Unavailable Error Response.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")'),
        ('ServerErrorResponse', r'(### 5XX - Server Error Response.*?--8<-- "src/spec/rest\.yaml:)\d+:\d+(")')
    ]
...
def main():
    ...
    # Get the script directory (docs/src/spec)
