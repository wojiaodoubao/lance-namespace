...
/**
 * Custom serializer for QueryRequest that handles the vector field as an untagged union. The server
 * expects the vector field as a direct array, not as an object with properties.
 */
public class QueryRequestSerializer extends JsonSerializer<QueryRequest> {

  @Override
  public void serialize(QueryRequest value, JsonGenerator gen, SerializerProvider provider)
      throws IOException {
    ...
    if (value.getVector() != null) {
      QueryRequestVector vectorObj = value.getVector();
    ...
    if (value.getFullTextQuery() != null) {
      ...
      QueryRequestFullTextQuery ftq = value.getFullTextQuery();
